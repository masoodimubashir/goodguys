...
const ChallanPdf = ({ client, challan_refrence }) => {
  const challanReference = challan_refrence || {};
  const challans = challanReference.challans || [];

  // Process challan data
  let challanSubtotal = 0;
  const serviceCharge = parseFloat(challanReference.service_charge) || 0;

  const hasVisiblePrices = challans.some(item => item.is_price_visible);

  const processedItems = challans.map(item => {
  ...
  const showPrices = hasVisiblePrices;
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          ...
          <View>
            <Text style={{ fontSize: 24, fontWeight: 'black', color: colors.secondary }}>CHALLAN</Text>
          </View>
        </View>
          ...
          <View style={styles.column}>
            <View style={styles.infoRow}>
              ...
              <Text style={styles.value}>
                {challanReference.created_at ? 
                  new Date(challanReference.created_at).toLocaleDateString() : '-'}
              </Text>
            </View>
            <View style={styles.infoRow}>
            ...
            <View style={styles.infoRow}>
              ...
              <Text style={styles.value}>{client?.site_name || '-'}</Text>
            </View>
          </View>
        </View>
          ...
          <View style={styles.challanSection}>
            <View style={styles.challanHeaderSection}>
              <Text style={styles.challanTitle}>
                Challan #{challanReference.challan_number || 'N/A'}
              </Text>
              {hasVisiblePrices && (
                <Text style={styles.challanTitle}>
            ...
            {hasVisiblePrices && (
              <>
                <View style={styles.serviceChargeRow}>
                  ...
                  <Text style={styles.value}>₹{challanSubtotal.toFixed(2)}</Text>
                </View>
                <View style={styles.serviceChargeRow}>
                  <Text style={styles.label}>Service Charge ({serviceCharge}%):</Text>
                  <Text style={styles.value}>₹{serviceChargeAmount.toFixed(2)}</Text>
                </View>
                <View style={[styles.serviceChargeRow, { backgroundColor: colors.primary }]}>
                  ...
                  <Text style={[styles.value, { color: 'white', fontWeight: 'bold' }]}>₹{challanTotal.toFixed(2)}</Text>
                </View>
              </>
            )}
          </View>
        ...
        {showPrices && (
          <View style={styles.totals}>
            ...
            <View style={[styles.totalRow, styles.grandTotal]}>
              <Text style={{ ...styles.label, fontWeight: 'black' }}>TOTAL AMOUNT:</Text>
        ...
        <View style={styles.footer}>
          <Text>Delivery Challan - Goods delivered as per above details</Text>
